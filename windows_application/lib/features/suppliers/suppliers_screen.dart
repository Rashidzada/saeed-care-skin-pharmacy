...
class _SuppliersScreenState extends State<SuppliersScreen> {
  ...
  Future<void> _openDialog([Map<String, dynamic>? item]) async {
    final changed = await showDialog<bool>(
      context: context,
      builder: (_) => _SupplierDialog(initial: item),
    );
  ...
  Future<void> _delete(Map<String, dynamic> item) async {
    ...
    try {
      await context
          ...
          .deleteSupplier((item['id'] as num).toInt());
      if (!mounted) {
        return;
      }
            ...
            children: [
              Expanded(
                child: DebouncedSearchField(
                  hintText: 'Search suppliers by name, contact, phone, or email...',
                  onChanged: (value) {
                    setState(() {
                      _search = value;
                      _page = 1;
                ...
                SingleChildScrollView(
                  ...
                  child: DataTable(
                    columns: [
                      ...
                      const DataColumn(label: Text('Contact Person')),
                      const DataColumn(label: Text('Phone')),
                      const DataColumn(label: Text('Email')),
                      const DataColumn(label: Text('Medicines')),
                        ...
                        .map(
                          (item) => DataRow(
                            cells: [
                              ...
                              DataCell(
                                Text(item['contact_person']?.toString().isNotEmpty == true
                              ...
                              DataCell(Text(item['email']?.toString().isNotEmpty == true
                                  ? item['email'].toString()
                                  : '-')),
                              DataCell(Text('${item['medicine_count'] ?? 0}')),
                              DataCell(
                                StatusBadge(
                                  label:
                                      (item['is_active'] as bool? ?? false)
                                          ? 'ACTIVE'
                                          : 'INACTIVE',
                                  background: (item['is_active'] as bool? ?? false)
                                      ? const Color(0xFFDCFCE7)
                                      : const Color(0xFFFEE2E2),
                                  foreground: (item['is_active'] as bool? ?? false)
...
class _SupplierDialogState extends State<_SupplierDialog> {
  ...
  void initState() {
    ...
    _contactController = TextEditingController(
      text: widget.initial?['contact_person']?.toString() ?? '',
    );
    _phoneController =
